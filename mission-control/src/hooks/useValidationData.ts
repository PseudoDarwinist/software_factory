...
export function useValidationData(
  options: UseValidationDataOptions
): UseValidationDataReturn {
  ...
  const [evidenceByCheckId, setEvidenceByCheckId] = useState<
  ...
  useEffect(() => {
    ...
    unsubStatus = validationWS.onStatusChange((s) =>
      setConnection({
          ...
          if (prevStatus && prevStatus !== c.status) {
            ...
            try {
              if (
                ...
                new Notification(`Validation ${c.status}`, {
                  body: `${c.name}`,
                });
              }
            } catch {}
    ...
    unsubEvidence = validationWS.subscribe("validation.evidence", (payload) => {
      ...
      setEvidenceByCheckId((prev) => {
        const merged = mergeEvidence(
          ...
          events
            .filter(Boolean)
            .map((e) => ({ checkId: e.checkId, evidence: e.evidence || {} }))
        );
